(function () {
  ...
  function safeUrl(value) {
    try {
      ...
      Array.from(url.searchParams.keys()).forEach(function (key) {
        if (/email|phone|name|address|message|comment|token|password/i.test(key))
  ...
  function capture() {
    ...
    var campaignChanged =
      hasCampaign(touch) &&
      JSON.stringify(touch) !== JSON.stringify(attribution.latest_touch || {});
    ...
    CAMPAIGN_KEYS.forEach(function (key) {
      var value =
        touch[key] || (attribution.latest_touch || {})[key] || (attribution.first_touch || {})[key];
  ...
  function decorate(attribution) {
    document
      ...
      .forEach(function (element) {
        var attribute =
          element.tagName === "A" ? "href" : element.hasAttribute("data-src") ? "data-src" : "src";
        ...
        try {
          ...
          });
          {
            ...
            Object.keys(fields).forEach(function (key) {
              if (fields[key]) url.searchParams.set(key, fields[key]);
            });
          }
